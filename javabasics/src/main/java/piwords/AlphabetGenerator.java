    ...
    public static char[] generateFrequencyAlphabet(int base,
        ...
        for (String word: trainingData) {
            for (int i=0; i < word.length(); i++) {
                ...
                if (!Character.isAlphabetic(chr)) continue;
                charFreq.replace(chr, charFreq.get(chr) + 1);
                totalChars ++;
            }
        }

        double cumProb = 0.0;
        HashMap<Character, Double> charProb = new HashMap<Character, Double>();
        for (Map.Entry<Character, Integer> entry: charFreq.entrySet()) {
            cumProb += (double) entry.getValue() / totalChars;
            charProb.put(entry.getKey(), cumProb);
        }

        char[] alph = new char[base];
        int k = 0;
        for (Map.Entry<Character, Double> entry: charProb.entrySet()) {
            double count = entry.getValue() * base;
            while (k < count) {
                alph[k] = entry.getKey();
                k += 1;
            }
